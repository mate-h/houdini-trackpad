...
MTDeviceRef
    MTDeviceCreateFromGUID(uuid_t); // GUID's compared by pointer, not value!
...
OSStatus MTDeviceGetDeviceID(MTDeviceRef, uint64_t *)
    __attribute__((weak_import)); // no 10.5
...
typedef void (*MTFrameCallbackFunction)(MTDeviceRef device, MTTouch touches[],
                                        size_t numTouches, double timestamp,
                                        size_t frame);
...
typedef void (*MTFrameCallbackRefconFunction)(MTDeviceRef device,
                                              ...
                                              void *refcon);
void MTRegisterContactFrameCallbackWithRefcon(MTDeviceRef,
                                              MTFrameCallbackRefconFunction,
                                              void *refcon);
void MTUnregisterContactFrameCallback(MTDeviceRef,
                                      MTFrameCallbackRefconFunction);

typedef void (*MTPathCallbackFunction)(MTDeviceRef device, long pathID,
                                       long state, MTTouch *touch);
